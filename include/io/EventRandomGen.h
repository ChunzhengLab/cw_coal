...
class EventRandomGen {
 public:
  ...
  enum SamplingMode { kToyMode, kSampleFromFile };
  /**
   * 构造函数
   ...
   */
  explicit EventRandomGen(const std::string& histFilePath = std::string(DATA_INSTALL_DIR) + "/dist_parton_afART.root");
   ...
   */
  void GenerateEvent(Event& out, int nParts = -1, int sumBaryonNumber = 0, SamplingMode mode = kSampleFromFile) const;
