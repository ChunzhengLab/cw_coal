...
public:
    ...
    static void Assign(Event& event) {
        ...
        for (Parton* parton : event.GetPartons()) {
            uidToPartonMap[parton->UniqueID()] = parton;
        }
        ...
        for (Hadron* hadronPtr : hadronList) {
            ...
            for (auto uid : constituentUIDs) {
                quarkFlavorCodes.push_back(uidToPartonMap[uid]->GetPID());
            }
            // diagonal light mesons are size==2 & q0==-q1 & |q|<=2
            if (!(quarkFlavorCodes.size() == 2 &&
                  quarkFlavorCodes[0] == -quarkFlavorCodes[1] &&
        ...
        if (!diagonalMesonIndices.empty()) {
            ...
            PIDInference::BatchAssignDiagonalLightMesons(
                diagonalMesonMasses,
                diagonalMesonPDGs,
                numChargedPions,
                numChargedRhos
