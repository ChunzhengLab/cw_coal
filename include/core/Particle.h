...
class Particle : public TObject {
 public:
  ...
  Particle(double x, double y, double z, double px, double py, double pz, double baryonNumber)
      : fUniqueID(sNextID++), fX(x), fY(y), fZ(z), fPx(px), fPy(py), fPz(pz), fBaryonNumber(baryonNumber) {
  }
  ...
  double GetBaryonNumber() const {
    return fBaryonNumber;
  }
  void SetBaryonNumber(double b) {
    fBaryonNumber = b;
  }
...
class Parton : public Particle {
 public:
  ...
  Parton(double x, double y, double z, double px, double py, double pz, double baryonNumber)
...
class Hadron : public Particle {
 public:
  ...
  Hadron(double x, double y, double z, double px, double py, double pz, double baryonNumber, double formationDistance)
      : Particle(x, y, z, px, py, pz, baryonNumber), fFormationDistance(formationDistance) {
  }

  void SetFormationDistance(double d) {
    fFormationDistance = d;
  }
  double GetFormationDistance() const {
    return fFormationDistance;
  }

  void AddConstituentID(unsigned int id) {
    fConstituentIDs.push_back(id);
  }
  const std::vector<unsigned int>& GetConstituentIDs() const {
    return fConstituentIDs;
  }
  void SetAfterBurnedFlag(bool b) {
    isAfterBurned = b;
  };
  bool IsAfterBurned() const {
    return isAfterBurned;
  }
