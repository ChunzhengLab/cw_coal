...
static int InferPID(const std::vector<int>& quarks,
                    double mass);

// Batch-assign light diagonal mesons using event-level charged π and ρ counts
static void BatchAssignDiagonalLightMesons(const std::vector<double>& masses,
...
private:
  ...
  static int InferPIDWithRNG(const std::vector<int>& quarks,
                      double mass,
                      double rnd,
                      double vpratio,
                      double rrhopi,
