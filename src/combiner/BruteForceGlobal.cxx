...
struct Candidate {
  ...
  Candidate(double dist, bool isB, size_t a, size_t b) : distance(dist), isBaryon(isB), idxA(a), idxB(b), idxC(0) {
  }
...
std::vector<Hadron*> BruteForceGlobal::Combine(const std::vector<Parton*>& partons) {
  ...
  for (size_t i = 0; i < partons.size(); ++i) {
    ...
    for (size_t j = i + 1; j < partons.size(); ++j) {
      ...
      for (size_t k = j + 1; k < partons.size(); ++k) {
        ...
        double dist = partons[i]->DistanceTo(*partons[j]) + partons[i]->DistanceTo(*partons[k]) +
  ...
  for (const auto& candi : candidates) {
    if (candi.isBaryon) {
      ...
      hadrons.push_back(h);
      used.insert(candi.idxA);
      used.insert(candi.idxB);
      used.insert(candi.idxC);
      a->MarkUsed();
      b->MarkUsed();
      c->MarkUsed();
    } else {
