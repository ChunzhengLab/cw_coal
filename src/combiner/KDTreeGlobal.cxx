...
std::vector<Hadron*> KDTreeGlobal::Combine(const std::vector<Parton*>& partons) {
  ...
  for (const auto& candi : candidates) {
    if (candi.isBaryon) {
      ...
      double E1 = std::sqrt(candi.a->Px() * candi.a->Px() + candi.a->Py() * candi.a->Py() +
                            candi.a->Pz() * candi.a->Pz() + m1 * m1);
      double m2 = candi.b->GetMassFromPDG();
      double E2 = std::sqrt(candi.b->Px() * candi.b->Px() + candi.b->Py() * candi.b->Py() +
                            candi.b->Pz() * candi.b->Pz() + m2 * m2);
      double m3 = candi.c->GetMassFromPDG();
      double E3 = std::sqrt(candi.c->Px() * candi.c->Px() + candi.c->Py() * candi.c->Py() +
                            candi.c->Pz() * candi.c->Pz() + m3 * m3);
      ...
      auto* h = new Hadron(
          x, y, z, px, py, pz,
          std::round(candi.a->GetBaryonNumber() + candi.b->GetBaryonNumber() + candi.c->GetBaryonNumber()), formation);
      h->SetMass(invMass);
      h->SetPosition(x, y, z);
      ...
      auto* h = new Hadron(x, y, z, px, py, pz, std::round(candi.a->GetBaryonNumber() + candi.b->GetBaryonNumber()),
                           formation);
      h->SetMass(invMass);
      h->SetPosition(x, y, z);
      h->AddConstituentID(candi.a->UniqueID());
