...
int PIDInference::InferQuarkoniumPDG(int flavor, double mass) {
  if (flavor == 4) {  // charm
    ...
    static const std::vector<int> pdgList = {441,    443,   10441,  20443,   10443,   445,    100441,
    ...
    if (std::abs(mass - PhysicsConstants::GetMass(pdg).value_or(0.0)) >
        std::abs(mass - PhysicsConstants::GetMass(pdg + 2).value_or(0.0))) {
      pdg += 2;
      if (std::abs(mass - PhysicsConstants::GetMass(553).value_or(0.0)) >
          std::abs(mass - PhysicsConstants::GetMass(10551).value_or(0.0)))
        pdg = 10551;
    }
    return pdg;
  } else if (flavor == 6) {  // top
    return (std::abs(mass - PhysicsConstants::GetMass(661).value_or(0.0)) >
            std::abs(mass - PhysicsConstants::GetMass(663).value_or(0.0)))
               ? 663
               : 661;
  }
...
void PIDInference::BatchAssignDiagonalLightMesons(const std::vector<double>& masses, std::vector<int>& outPDG,
...
int PIDInference::InferPIDWithRNG(const std::vector<int>& quarks, double mass, double rnd, double vpratio,
                                  double rrhopi, double romrho0) {
  if (quarks.size() == 2) {
    ...
    if (spin == 1 && std::abs(pdg % 10) == 1) { pdg += 2 * ((pdg > 0) ? 1 : -1); }
    return pdg;
...
int PIDInference::InferPID(const std::vector<int>& quarks, double mass) {
  ...
  return InferPIDWithRNG(quarks, mass, rnd, kDefaultVPRatio, kDefaultRhoPiRatio, kDefaultOmegaRhoRatio);
}
