...
bool runTest(const std::vector<int>& quarks, double mass, int expected, const std::string& desc) {
  int result = PIDInference::InferPID(quarks, mass);
  bool pass = (result == expected);
  std::cout << (pass ? "[PASS]" : "[FAIL]") << " " << desc << " => Got " << result << ", expected " << expected << "\n";
  return pass;
}
...
int main() {
  ...
  // Statistical test for random outcomes
  {
    ...
    for (auto& kv : counts) {
      std::cout << "  PDG " << kv.first << " (" << pdgName(kv.first) << "): " << (100.0 * kv.second / N) << "%\n";
    }
  }
