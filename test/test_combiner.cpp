...
static const char* kPartonHistFile =
    std::getenv("CW_COAL_PARTON_HIST") ? std::getenv("CW_COAL_PARTON_HIST") : defaultHistPath.c_str();

// Lambda to deep-copy an Event (clone all Partons)
auto cloneEvent = [](const Event& src) {
  Event clone;
  for (auto* p : src.GetPartons()) { clone.AddParton(new Parton(*p)); }
  return clone;
};

// Run a single test: receives a reference to base Event, deep-copies partons, applies combiner, and prints stats
void RunTest(const std::string& label, CombinerBase& combiner, Event& event) {
  ...
  if (totalUsed != localPartons.size()) {
    std::cerr << " Consistency check failed: "
              << "3 * baryons + 2 * mesons + unused = " << totalUsed << " != " << localPartons.size() << "\n";
  } else {
...
int main() {
  ...
  for (auto& tc : tests) {
    ...
    for (int ie = 0; ie < nEvents; ++ie) {
      ...
      std::cout << "Using base event " << (ie + 1) << " of " << nEvents << " with " << evt.GetPartons().size()
  ...
  for (auto& evt : baseEvents) { evt.Reset(); }
